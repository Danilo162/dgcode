<div class="dropdown">
    ...
    <div class="dropdown-menu">
      {{--  <a class="dropdown-item"   href="{{route('settings.permissions.assignations',['id'=>$rs->id])}}"
        ...
        >--}}
        <a class="dropdown-item" onclick="loadModal('{{route('consoles.settings.permissions.form')}}','{{json_encode(["id"=>$rs->id])}}','FORMULAIRE DE PERMISSIONS')"  href="javascript:void(0);"
        ><i class="bx bx-edit-alt me-1 text-success"></i> Modifier</a
        >
        @if($rs->total==0)
            <a class="dropdown-item"
               onclick="deleteAllShowConfirme('{{route('consoles.settings.permissions.destroy')}}','{{$rs->id}}',launchReloadDataTable);" href="javascript:void(0);"
