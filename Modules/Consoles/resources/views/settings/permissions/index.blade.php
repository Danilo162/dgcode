        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Permissions <span class="text-primary  total_count">0</span> </li>
                </ol>
            </nav>
        </div>
     {{--   &nbsp; <h6 class="mb-0 text-uppercase">DataTable Example</h6>--}}
        <div class="ms-auto">
            <button type="button" onclick="loadModal('{{route('consoles.settings.permissions.form')}}','{{json_encode([])}}','FORMULAIRE DE PERMISSION')" class="btn btn-primary"> <i class="bx bx-plus"></i> Nouveau</button>
        </div>
    </div>
  ...
  {{--  <script src="{{asset('select2-v4_1/select2.full.min.js')}}" defer></script>--}}
    <script>
        ...
        function launchReloadDataTable() {
            ...
            table.destroy();
            route = '{{route('consoles.settings.permissions.getall')}}';
            title = "Liste des permissions"
            columns =  ['name','comment','action'];
            postColumns =  ['l','l','c'];
