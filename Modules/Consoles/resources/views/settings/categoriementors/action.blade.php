<div class="dropdown">
    ...
    <div class="dropdown-menu">
      {{--  <a class="dropdown-item"   href="{{route('settings.permissions.assignations',['id'=>$rs->id])}}"
        ><i class="bx bx-arrow-to-right me-1 text-primary"></i> Gerer ses permissions</a
        >--}}
        <a class="dropdown-item" onclick="loadModal('{{route('consoles.settings.categoriementors.form')}}','{{json_encode(["id"=>$rs->id])}}','FORMULAIRE DE CATEGORIE MENTOR')"  href="javascript:void(0);"
        ><i class="bx bx-edit-alt me-1 text-success"></i> Modifier </a
        >
        @if($rs->total==0)
            <a class="dropdown-item"
               onclick="deleteAllShowConfirme('{{route('consoles.settings.categoriementors.destroy')}}','{{$rs->id}}',launchReloadDataTable);" href="javascript:void(0);"
